...
{
	/*
	output << "Cantidad de cilindros: " << cylinders.size() << endl;
	for(unsigned int i = 0; i < cylinders.size(); ++i)
		output << "cantidad de coordenadas: " << cylinders[i].getNumCells() << endl;
*/
	for(unsigned int i = 0; i < cylinders.size(); ++i)
	{
		output << cylinders[i].getCM(0);
		for(unsigned int j = 1; j < cylinders[i].getNumCells(); ++j)
			output << sep << cylinders[i].getCM(j);
		//output << endl;
